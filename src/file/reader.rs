...
/// 读取文件的接口定义
#[enum_dispatch]
pub trait Reader: Sized + ReaderBase {
  /// 给定需要被读取的文件路径，创建新的 reader
  async fn open(path: &Path, config: Config) -> Result<Self>;
}

/// 往头部方向读取新的内容，将完整的行发送出去
pub async fn read_head_lines(buffer: &mut Vec<u8>, state: &mut State) -> Result<()> {
  // 向前读取一部分内容
  // 这里分成三个部分，头部可能是不完整的一行，中间部分可以发送至头部插入，
  // 尾部和之前的缓存能连成完整的一行，第一个发送出去拼接
  if let Some(parts) = read_buffer(buffer, state, ReadDirection::Head).await? {
    ...
    // 处理头部
    update_head_line(state, parts.head).await?;
  }
...
pub async fn read_tail_lines(buffer: &mut Vec<u8>, state: &mut State) -> Result<()> {
  // 向后读取一部分内容，
  // 这里分成三个部分，头部和之前的缓存连在一起，组成完整的行。中间部分是完整的行。
  // 尾部如果包含换行符，则是完整的行，否则需要暂存起来，等待下次读取时拼接。
  // 我们不考虑只读到一行，仍然不完整的情况，此处只能截断处理（也就是直接发送出去）。
  if let Some(parts) = read_buffer(buffer, state, ReadDirection::Tail).await? {
    if let Some(tail_part) = parts.tail {
      ...
      // 处理尾部
      update_tail_line(state, tail_part, parts.tail_is_end).await?;
    } else {
      // 尾部不存在，意味着我们只读到一行
      if let Some(head_part) = parts.head {
        // 作为尾部进行处理
        update_tail_line(state, head_part, parts.tail_is_end).await?;
      }
    }
  }
...
pub async fn update_head_line(head_state: &mut State, buffer: Option<&[u8]>) -> Result<()> {
  match buffer {
    Some(buffer) => {
      // 追加新内容
