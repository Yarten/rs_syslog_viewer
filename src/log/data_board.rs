...
pub struct DataBoard {
  /// 从日志中发现的标签集合，有序，用于过滤日志
  ordered_tags: RwLock<BTreeSet<String>>,
...
impl DataBoard {
  ...
  pub fn update_tag(&self, new_tag: &String) {
    let mut ordered_tags = self.ordered_tags.write().unwrap();
    ...
    if !hashed_tags.contains(new_tag) {
      ordered_tags.insert(new_tag.clone());
      hashed_tags.insert(new_tag.clone());
    }
  }

  /// 获取所有的日志标签（有序）
  pub fn get_tags(&'_ self) -> RwLockReadGuard<'_, BTreeSet<String>> {
    self.ordered_tags.read().unwrap()
  }
}
